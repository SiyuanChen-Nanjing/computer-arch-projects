	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  struct cache_blk_t *blk, *repl, *repl_temp;
  int lat = 0;
  ...
  if (cp->hsize)
  {
    ...
    unlink_htab_ent(cp, &cp->sets[set], repl_temp);
  }
...
/* replace a block in the cache with an input block */
int         /* non-zero if access would hit */
cache_replace(struct cache_t *cp,   /* cache instance to probe */
    md_addr_t addr,
    md_addr_t repl_tag) 
{
  md_addr_t tag = CACHE_TAG(cp, addr);
  ...
  else
  {
   ...
   blk=blk->way_next)
    ...
    {
      /*
        printf("cache_replace, blk tag: %d\n", blk->tag);
        printf("cache_replace, repl tag: %d\n", repl_tag);
        printf("cache_replace, set: %d\n", set);
        printf("cache_replace, addr: %d\n", CACHE_MK_BADDR(cp, repl_tag, set));
        printf("found!\n");
      */
        blk->tag = repl_tag;
        return TRUE;
