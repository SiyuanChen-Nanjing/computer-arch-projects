...
/* victim cache, entry level data cache */
...
dl1_access_fn(enum mem_cmd cmd,		/* access cmd, Read or Write */
	      ...
	      tick_t now,
        md_addr_t repl_tag)		/* time of access */
{
  unsigned int lat;

  //printf("blk addr: %d\n", CACHE_MK_BADDR(cache_dl1, blk->tag, CACHE_SET(cache_dl1, baddr)));
  //printf("baddr tag: %d\n",CACHE_TAG(cache_dl1, baddr));
  if (cache_victim) {
  	if (cache_probe(cache_victim, baddr) != 0) {
      ...
      // write-back
      if (cmd == Write) {
        cache_access(cache_victim, cmd, baddr, NULL, bsize, now, NULL, NULL);
      }
      // read, swap
      else {
        if (repl_tag != -1) {
          md_addr_t set = CACHE_SET(cache_dl1, baddr);
          cache_replace(cache_victim, baddr, CACHE_TAG(cache_victim, CACHE_MK_BADDR(cache_dl1, repl_tag, set)));
          cache_access(cache_victim, cmd, CACHE_MK_BADDR(cache_dl1, repl_tag, set), NULL, bsize, now, NULL, NULL);
        }
      }
      return 0;
  	}
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  fatal("bad victim D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      cache_victim = cache_create(name, nsets, bsize, /* balloc */FALSE,
             /* usize */0, assoc, cache_char2policy(c),
             victim_access_fn, /* hit lat */cache_victim_lat);
  }
      ...
      if (RUU[RUU_head].ea_comp)
	{
	  ...
	  if ((MD_OP_FLAGS(LSQ[LSQ_head].op) & (F_MEM|F_STORE))
	      ...
	      if (fu)
		{
          ...
          //if (cache_probe(cache_dl1, (LSQ[LSQ_head].addr&~3))==0) printf("l1 miss: %d\n", CACHE_TAG(cache_dl1, (LSQ[LSQ_head].addr&~3)));
		      lat =
      ...
      if (RSLINK_VALID(node))
	{
	  ...
	  if (rs->in_LSQ
	      ...
	      if (MD_OP_FUCLASS(rs->op) != NA)
		{
          ...
          //if (cache_probe(cache_dl1, (rs->addr & ~3))==0) printf("l1 miss: %d\n", CACHE_TAG(cache_dl1, (rs->addr & ~3)));
				  load_lat =
